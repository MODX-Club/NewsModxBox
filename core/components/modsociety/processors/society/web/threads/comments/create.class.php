...
class modSocietyWebThreadsCommentsCreateProcessor extends modSocietyWebThreadsUpdateProcessor{
    ...
    public function beforeSave(){
        ...
        */
        ...
        // exit;
        /*
            Для импорта из ливстрита оставляем возможность переопределения данных
        */
        if($id = (int)$this->getProperty('comment_id')){
            $comment->set('id', $id);
        }
        ...
        if($comment_date = $this->getProperty('comment_date')){
            $comment->set('createdon', $comment_date);
        }
        ...
        $this->object->set('comment_id', $comment->id);
